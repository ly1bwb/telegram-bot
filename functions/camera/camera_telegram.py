...
async def lower_camera(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        web_file = urllib.request.urlopen(lower_camera_url, timeout=30)
...
async def rig_camera(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        web_file = urllib.request.urlopen(rig_camera_url, timeout=30)
        await context.bot.send_photo(
            chat_id=update.effective_chat.id,
            message_thread_id=update.effective_message.message_thread_id,
            photo=web_file.read(),
...
async def window_camera(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    log_func("window_camera()", update)
    try:
        web_file = urllib.request.urlopen(window_camera_url, timeout=30)
        await context.bot.send_photo(
            chat_id=update.effective_chat.id,
            message_thread_id=update.effective_message.message_thread_id,
            photo=web_file.read(),
...
async def main_camera(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        web_file = urllib.request.urlopen(main_camera_url, timeout=30)
        await context.bot.send_photo(
            chat_id=update.effective_chat.id,
            message_thread_id=update.effective_message.message_thread_id,
            photo=web_file.read(),
...
async def roof_camera(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    try:
        web_cam_url = urllib.request.urlopen(roof_camera_url, timeout=30)
        web_cam_html = web_cam_url.read()
        parser = webcam_parser()
        parser.feed(web_cam_html.decode("utf-8"))
        web_file = urllib.request.urlopen(parser.roof_camera_img, timeout=30)
        await context.bot.send_photo(
            chat_id=update.effective_chat.id,
            message_thread_id=update.effective_message.message_thread_id,
            photo=web_file.read(),
